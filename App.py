...
def run():
    ...
    if choice == 'User':
        ...
        st.markdown("""
        <div style='padding: 20px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #667eea; margin-bottom: 20px;'>
            <h3 style='color: #333; margin: 0 0 10px 0;'>Upload Your Resume</h3>
            <p style='color: #666; margin: 0;'>Get professional analysis, identify gaps, and receive personalized recommendations.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            act_mail = st.text_input('Email*', placeholder='your@email.com')
        with col3:
        ...
        # Get location data with error handling for SSL issues
        try:
            import ssl
        ...
        pdf_file = st.file_uploader("Upload your resume (PDF format)", type=["pdf"], label_visibility="visible")
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                st.markdown("""
                <div style='padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 10px; margin: 20px 0;'>
                    <h2 style='color: white; margin: 0;'>Resume Analysis Report</h2>
                </div>
                """, unsafe_allow_html=True)
                
                st.markdown(f"""
                <div style='padding: 15px; background: #e8f5e9; border-radius: 8px; border-left: 4px solid #4caf50; margin-bottom: 20px;'>
                    <h3 style='color: #2e7d32; margin: 0;'>Hello, {resume_data.get('name', 'User')}!</h3>
                    <p style='color: #666; margin: 5px 0 0 0;'>Here is your comprehensive resume analysis</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                try:
                    ...
                    with col2:
                        st.metric("Contact", resume_data['mobile_number'])
                        st.metric("Degree", str(resume_data.get('degree', 'N/A')))
                    st.metric("Resume Pages", str(resume_data['no_of_pages']))
